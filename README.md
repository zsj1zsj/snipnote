...
- “查看并批注”页支持 AI 功能：
  - `AI 总结`：点击按钮自动生成摘录摘要，卡片列表优先显示总结内容
  - `建议标签`：点击按钮自动分析内容并推荐合适的标签，自动合并到现有标签
...
- **可配置优于硬编码** - 站点解析规则、快捷键、AI prompt 等均可配置文件管理
...
├── ai/            # LLM 插件层
├── rss/           # RSS 模块（预留）
├── plugins/        # 插件系统（未来支持复习算法、AI 动作、导出器等扩展）
└── 根目录         # 向后兼容层
```
